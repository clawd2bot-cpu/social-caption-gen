...
const AGENT_SPEED = 4;
const LOS_ELIMINATE_TIME = 0.8; // seconds of continuous LOS to eliminate
...
function clampObstacleToArena(obs: ObstacleData): void {
  const half = ARENA_SIZE / 2 - obs.size.x / 2 - 0.5;
  const halfZ = ARENA_SIZE / 2 - obs.size.z / 2 - 0.5;
  obs.pos.x = Math.max(-half, Math.min(half, obs.pos.x));
...
function BlobCharacter({ agent }: { agent: Agent }) {
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* LOS warning indicator - red ring when being seen */}
...
// Catch/elimination effect
function CatchEffect({ position }: { position: Vec3 }) {
  const ref = useRef<THREE.Mesh>(null);
  ...
  useFrame(() => {
    ...
    (ref.current.material as THREE.MeshStandardMaterial).opacity = 1 - elapsed;
  });

  return (
    <mesh ref={ref} position={[position.x, 0.5, position.z]} rotation={[-Math.PI / 2, 0, 0]}>
      <ringGeometry args={[0.3, 0.6, 16]} />
      <meshStandardMaterial color="#ffaa00" transparent opacity={1} emissive="#ffaa00" emissiveIntensity={2} side={THREE.DoubleSide} />
    </mesh>
  );
}
...
export default function AgentManager({
  ...
  const catchEffects = useRef<Vec3[]>([]);
  ...
  useFrame((_, delta) => {
    ...
    if (gameState === "seeking") {
      ...
      if (remaining <= 0 || redPositions.length === 0) {
        const survived = redAlive.length;
        const caught = NUM_PER_TEAM - survived;

        for (const r of reds) {
          r.brain.fitness = r.found ? 0 : 10;
        }
        for (const b of blues) {
          b.brain.fitness = caught * 2;
        }
    ...
    for (const agent of agents) {
      ...
      // During hide phase, blue team is frozen
      ...
      const pushStrength = outputs[2]; // Also used for pushing
      ...
      if (Math.abs(pushStrength) > 0.3) {
        for (const obs of obstacles) {
          ...
          const distToObs = Math.sqrt(
            (agent.position.x - obs.pos.x) ** 2 +
            (agent.position.z - obs.pos.z) ** 2
          );
          if (distToObs < PUSH_RADIUS + Math.max(obs.size.x, obs.size.z) / 2) {
    ...
    // Line of sight checks & elimination (seeking phase only)
        ...
        for (const blue of blues) {
          ...
          if (canSee) {
            ...
            if (newTime >= LOS_ELIMINATE_TIME) {
              red.found = true;
              catchEffects.current.push({ ...red.position });
              setStats((s) => ({ ...s, redAlive: s.redAlive - 1 }));
              losTimers.current.delete(key);
            }
          } else {
            const key = `${blue.id}-${red.id}`;
            // Decay LOS timer when not in sight
            const current = losTimers.current.get(key) ?? 0;
            if (current > 0) {
  ...
  return (
    <group>
      ...
      {catchEffects.current.map((pos, i) => (
        <CatchEffect key={`catch-${i}`} position={pos} />
      ))}
